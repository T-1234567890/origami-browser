...
HTTPS-First is enabled by default in Settings → Privacy → Connection Security. Origami uses WebKit's public `preferredHTTPSNavigationPolicy` with `automaticFallbackToHTTP`: WebKit tries HTTPS for top-level HTTP navigation and automatically retries HTTP when the upgrade fails. Origami does not accept invalid certificates or implement its own request-replay fallback.

Bare addresses entered without a scheme use an HTTP starting URL when HTTPS-First is enabled, letting WebKit upgrade them before loading. Explicit `https://` addresses and HTTPS search URLs remain unchanged; they are not rewritten to HTTP on error.
...
Automated tests cover default/persistent preferences, changes on existing pages, private-page automatic fallback, bare-address routing, explicit-scheme preservation, and unverified certificate presentation. They use no accounts or remote websites. Before release, manually exercise a working HTTPS upgrade, an HTTP-only site, a downgrade redirect, an invalid/expired certificate, form submissions, back/forward, and the certificate sheet on supported macOS versions. Live upgrade/fallback and TLS outcomes are not established by policy tests.
