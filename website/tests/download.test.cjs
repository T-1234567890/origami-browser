...
async function render(data, ok = true) {
  const buttons = Array.from({length: 2}, () => ({innerHTML: 'Beta coming soon', attrs: {'aria-disabled': 'true', tabindex: '-1'}, removeAttribute(name) {delete this.attrs[name];}}));
...
test('all CTAs resolve the single relative metadata file', async () => {
  ...
  for (const button of result.buttons) {assert.equal(button.href, valid.downloadURL); assert.match(button.innerHTML, /^Download for Mac /); assert.deepEqual(button.attrs, {});}
});
...
test('placeholder, malformed data, failed requests, and temporary URLs stay disabled', async () => {
  for (const [data, ok] of [[{version:null, channel:null, downloadURL:null}, true], [valid, false], [null, true], [{...valid, channel:'stable'}, true], [{...valid, downloadURL:'https://temporary.example/archive.zip'}, true], [{...valid, downloadURL:valid.downloadURL + '?token=fixture'}, true]]) {
    for (const button of (await render(data, ok)).buttons) {assert.equal(button.href, undefined); assert.equal(button.attrs['aria-disabled'], 'true'); assert.equal(button.innerHTML, 'Beta coming soon');}
  }
});
test('HTML download links have no hardcoded target and metadata is valid', () => {
  ...
  assert.equal((html.match(/data-download[^>]*>Beta coming soon/g) || []).length, 2);
